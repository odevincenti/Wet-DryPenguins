...
int calibrating_function(unsigned char input, int repeating);
...
int (*const(FSM_ARRAY[]))(unsigned char, int) = {
    ...
    [CALIBRATING_STATE] = calibrating_function,
...
int init_state_function(unsigned char input, int repeating){
    ...
    case 'C': case 'c':
        ...
        if(!repeating){
            ...
            is_calibrating_minute = 1;
        }
        change_state(CALIBRATING_STATE, repeating);
        SEND_NUL;
        break;
    case '>':
        SEND_ACK;
...
int calibrating_function(unsigned char input, int repeating){
    ...
    case '>':
        SEND_ACK;
        if(!repeating){
            save_RTCCNT_here = RTCCNT;

            DISABLE_RTC;
            CLEAR_RTCIFG;

            WRITE_FRAM_ENABLE;
            calibrated_minute_value = save_RTCCNT_here;
            WRITE_FRAM_DISABLE;

            is_calibrating_minute = 0;
        }
        tx_string("\nCALIBRATED");
        SEND_INT(calibrated_minute_value);
        change_state(INIT_STATE, repeating);
        SEND_NUL;
        break;
