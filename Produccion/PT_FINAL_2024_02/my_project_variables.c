...
const unsigned int PENGUIN_ID = 232;

//30;//2344;
#define STARTING_CALIBRATED_MINUTE  2000
#define STARTING_WET_AND_DRY_PERIOD 10
#define STARTING_TEMPERATURE_PERIOD 10
...
volatile unsigned int wet_and_dry_period = STARTING_WET_AND_DRY_PERIOD; // dont make this 0
volatile unsigned int temperature_period = STARTING_TEMPERATURE_PERIOD; // dont make this 0

volatile unsigned int calibrated_minute_value = STARTING_CALIBRATED_MINUTE;//30;//2344;
volatile unsigned int remaining_mins_until_wet_and_dry = STARTING_WET_AND_DRY_PERIOD;
volatile unsigned int remaining_mins_until_temperature = STARTING_TEMPERATURE_PERIOD;
...
#pragma PERSISTENT(starting_date)
char starting_date[PENGUIN_NAME_SIZE+1] = "23-06-91 12:00:00";
...
void set_rtc_to_next_active(void){
    ...
    if(device_is_active && (wet_and_dry_is_active || temperature_is_active)){
        if(remaining_mins_until_wet_and_dry == 0){ // NO DEBERIA PASAR NUNCA AHORA
            UPDATE_PERSISTENT_VAR(remaining_mins_until_wet_and_dry, wet_and_dry_period);
        }
        if(remaining_mins_until_temperature == 0){ // NO DEBERIA PASAR NUNCA AHORA
            UPDATE_PERSISTENT_VAR(remaining_mins_until_temperature, temperature_period);
        }
